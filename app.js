...
(function() {
  ...
  const scale510 = document.getElementById('scale510');
  ...
  function notify(msg, type = 'info') { feedback.textContent = msg; feedback.className = 'alert ' + type; if (msg) show(feedback); else hide(feedback); }
  function debounce(fn, ms) { let t; return (...a) => { clearTimeout(t); t = setTimeout(() => fn(...a), ms); }; }
  function setBusy(b) { [fileInput, demoBtn, volumeSelect, canalSelect, outcomeSearch, scale510, resetBtn, exportBtn].forEach(el => { if (el) el.disabled = b; }); if (b) show(loadingEl); else hide(loadingEl); }
  ...
  function compute(pairs, rows, scaleMin) {
    ...
    for (const { label, imp, sat } of pairs) {
      ...
      const S = scaleMin === 5 ? Math.max(5, satMean) : satMean; // Satisfação ancorada em 5 quando ativado
      const OS = 2 * I - S;
  ...
  function renderChart(recs) {
    ...
    const xMin = 0, xMax = 10, yMin = (scale510.checked ? 5 : 0), yMax = 10;
  ...
  function update() {
    ...
    const scaleMin = scale510.checked ? 5 : 0;
    results = compute(pairs, filtered, scaleMin);
  ...
  async function loadFromJson() {
    ...
    try {
      ...
      update();
      notify('Dados carregados de data.json.');
    } catch (e) {
  ...
  function handleFile(file) {
    ...
    reader.onload = () => {
      try {
        ...
        populateFilters(rawRows);
        update();
        notify('Arquivo carregado com sucesso.');
      } catch (e) {
  ...
  function saveFilterState() {
    try {
      localStorage.setItem('odi.filters', JSON.stringify({ v: volumeSelect.value, c: canalSelect.value, s: outcomeSearch.value, t: scale510.checked }));
    } catch {}
  }
  function restoreFilterState() {
    try {
      ...
      if (typeof f.t === 'boolean') scale510.checked = f.t;
    } catch {}
  }
